...
def analysis(result, dfParams, c0=None, xx=None, cc=None, tt=None, plot=False,
    ...
    F_pre = np.mean(np.array([result[indices[i]].x[dfParams:dfParams*2]
                              for i in range(nbr)]), axis=0)
    F = F_pre - F_pre[0]
    ...
    FSTD = np.std(np.array([result[indices[i]].x[dfParams:dfParams*2]
                            for i in range(nbr)]), axis=0)
    ...
    F_best_pre = result[indices[0]].x[dfParams:dfParams*2]
    ...
    if bc is 'open1side':
        ...
        cc_ext = [np.concatenate((np.ones(1)*c0, cc[0]))] + cc[1:]
    ...
    if plot:
        # plotting profiles
        ps.plotConSkin(xx_ext, cc_ext, ccRes_ext, tt, locs=[1, 3], save=True,
                       start=1, end=-3, path=savePath)
...
def resFun(df, cc, tt, dfParams, deltaX=1, c0=None, verb=False, bc='reflective',
    ...
    dfParams: number of parameters to fit for D and F profiles
    '''
    ...
    if bc is 'open1side':
        # additional d and f for bin where c0 is constant
        d = np.concatenate((np.ones(1)*d[0], d))
        f = np.concatenate((np.ones(1)*f[0], f))
        ...
        b = np.append(c0*W10, np.zeros(W[:, 0].size-1))  # extra vector for open BCs
    ...
    k = 0
    for j in range(1, M):
        RR[k, :] = cc[j] - fp.calcC(cc[0], (tt[j] - tt[0]), T=T, Qb=Qb, bc=bc)[:-3]
        k += 1
    ...
    df_trunc = np.concatenate((alphaD*d[:-1], alphaF*f[:-1]))
...
def optimization(DRange, FRange, bnds, cc, tt, dfParams=None, deltaX=1,
                 c0=None, verb=0, bc='reflective', alpha=0):
    """
    ...
    optimize = ft.partial(resFun, dfParams=dfParams, cc=cc, tt=tt,
                          deltaX=deltaX, c0=c0, verb=funcVerb, bc=bc, alpha=alpha)
...
def main():
    ...
    cc0 = np.array([20.43270674, 20.39872678, 20.20967943, 19.82748853,
                   ...
                   3.06770597])
    cc = [cc0] + [cc[:, i] for i in range(1, cc[0, :].size)]  # now with c0
    xx = np.arange(190, step=10)  # new longer xx-vector

    # Overriding bounds and initial guesses for D and F
    params = dim+3  # 3 additional DF params now
    DBound = 1000
    FBound = 20
    ...
    if ana:
        ...
        analysis(np.array(res), dfParams=params, bc=bc_mode, c0=c0, xx=xx,
                 cc=cc, tt=tt, alpha=alpha, plot=True, per=0.1)
    ...
    for i in range(Runs):
        ...
        try:
            results.append(optimization(DRange=DInit[:, i], dfParams=params,
                                        FRange=FInit*np.ones(params),
    ...
    analysis(np.array(results), dfParams=params, bc=bc_mode, c0=c0, xx=xx,
             cc=cc, tt=tt, alpha=alpha, plot=True, per=0.1)
