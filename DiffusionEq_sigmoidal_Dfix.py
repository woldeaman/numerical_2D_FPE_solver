...
def analysis(result, c0, DSol=None, xx=None, cc=None, tt=None, plot=False,
             per=0.1, alpha=0, savePath=None):
    '''
    ...
    D_pre = np.array([DSol, D_pre])  # NOTE: fixing DSol
    F_preNoGauge = np.mean(np.array([result[indices[i]].x[1:3]
                                     for i in range(nbr)]), axis=0)
    F_pre = F_preNoGauge - F_preNoGauge[0]
    sigParamsDF_mean = np.mean(np.array([result[indices[i]].x[3:]
                                         for i in range(nbr)]), axis=0)
...
def resFun(df, DSol, cc, xx, tt, deltaX=1, c0=None, alpha=0, verb=False):
    '''
    ...
    d = df[0]
    d = np.array([DSol, d])  # NOTE: here fixing DSol
    f = df[1:3]  # letting F completely free
    # computing sigmoidal d and f profiles
    t_D, d_D, t_F, d_F = df[3], df[4], df[5], df[6]
...
def optimization(DRange, DSol, FRange, tdRange, bnds, cc, xx, tt, deltaX=1,
                 c0=None, alpha=0, verb=0):
    """
...
def main():
    ...
    if ana:
        ...
        analysis(np.array(res), DSol=DSol, c0=c0, xx=xx, cc=cc, tt=tt, plot=True,
                 per=0.1, alpha=alpha)
    ...
    analysis(np.array(results), DSol=DSol, c0=c0, xx=xx, cc=cc, tt=tt, plot=True, per=0.1,
             alpha=alpha)
