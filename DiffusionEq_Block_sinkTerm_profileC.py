...
def analysis(result, xx_DF, dx_dist, dfParams=None, dx_width=None, c0=None,
    ...
    source_terms = [np.concatenate((np.zeros(6), np.ones(8)*k, np.zeros(cc[1].size-8)))
...
def resFun(df, cc, xx, tt, dfParams, deltaX=1, dx_dist=None, dx_width=None,
    ...
    # only substract concentrations in bulk, first 8 bins, until 80µm
    source_terms = [np.concatenate((np.ones(8)*k, np.zeros(cc[1].size-8)))
                    for k in k_i]
